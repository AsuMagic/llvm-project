//===-- SmolISelLowering.cpp - Smol2 DAG Lowering Implementation ----------===//
//
...
//===----------------------------------------------------------------------===//
//
// This file defines the interfaces that Smol2 uses to lower LLVM code into a
// selection DAG.
//
//===----------------------------------------------------------------------===//
...
#define DEBUG_TYPE "smol-isellower"
...
SmolTargetLowering::SmolTargetLowering(const TargetMachine &TM,
  ...
  setStackPointerRegisterToSaveRestore(Smol::RPS);
...
void SmolTargetLowering::ReplaceNodeResults(SDNode *N,
...
SDValue SmolTargetLowering::LowerFormalArguments(
  ...
  CCState CCInfo(CallConv, isVarArg, DAG.getMachineFunction(), ArgLocs,
                 *DAG.getContext());
  CCInfo.AnalyzeFormalArguments(Ins, CC_Smol);
...
SDValue SmolTargetLowering::LowerCallResult(
  ...
  CCState CCInfo(CallConv, isVarArg, DAG.getMachineFunction(), RVLocs,
                 *DAG.getContext());
  CCInfo.AnalyzeCallResult(Ins, RetCC_Smol);
...
/// this.
void SmolTargetLowering::HandleByVal(CCState *State, unsigned &Size,
                                     llvm::Align Align) const {
  // Byval (as with any stack) slots are always at least 4 byte aligned.
  // FIXME: is .value() log2 etc.
  Align = llvm::Align(std::max(unsigned(Align.value()), 4U));
  ...
  unsigned Waste = (Smol::R6 - Reg) % AlignInRegs;
  ...
  unsigned ByValRegEnd = std::min<unsigned>(Reg + Size / 4, Smol::R6);
...
SmolTargetLowering::LowerReturn(SDValue Chain, CallingConv::ID CallConv,
  ...
  CCInfo.AnalyzeReturn(Outs, RetCC_Smol);
  ...
  return DAG.getNode(SmolISD::Ret, dl, MVT::Other, RetOps);
}
...
SDValue SmolTargetLowering::getGlobalAddressWrapper(SDValue GA,
